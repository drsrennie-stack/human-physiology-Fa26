...
(async () => {
  ...
  for (const f of FILES) {
    ...
    /* One Tab at a time, stopping at the first stop that paints a change.
       The old version pressed Tab three times and then looked. On a page with
       only two tab stops that walks off the end, activeElement returns to
       body, the two screenshots match, and it reported "no visible focus
       change" against a page whose focus ring was perfectly fine. */
    const focusProbe = await (async () => {
      const clip = { x: 0, y: 0, width: 900, height: 620 };
      const before = await p.screenshot({ clip });
      let changed = false, focused = null, stop = 0;
      for (let i = 1; i <= 6; i++) {
        ...
        const here = await p.evaluate(() => {
          ...
          return { tag: a.tagName.toLowerCase(), outlineWidth: cs.outlineWidth,
                   outlineColor: cs.outlineColor };
        });
        if (!here) break;              // walked off the end of the page
        const after = await p.screenshot({ clip });
        if (!before.equals(after)) { changed = true; focused = here; stop = i; break; }
        focused = here;
      }
      return { changed, focused, stop };
    })();
