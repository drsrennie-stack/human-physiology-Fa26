...
def page(n, opens, closes, title, part):
    ...
    learn = (a('lecture-week.html?week=%d' % n, 'Lectures, in order', True)
             ...
             + a('sheets/BIO005-note-sheet-week-%s.pdf' % nn, 'Competency Study Guide (PDF)')
             + (a('week-%s-notesheet-prompts.html' % nn, 'Competency Study Guide questions') if has_prompts else ''))
    retrieve = (a('note-sheet.html?week=%d' % n, 'Competency Study Guide, second color')
