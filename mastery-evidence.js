...
(function () {
  ...
  function fromCards() {
    ...
    Object.keys(st.topics).forEach(function (tid) {
      ...
      if (!topicEntry || !topicEntry.comps || !topicEntry.comps.length) return;
      var cards = (st.topics[tid] && st.topics[tid].cards) || {};
      Object.keys(cards).forEach(function (cid) {
        /* CARD LEVEL FIRST, TOPIC LEVEL AS THE FALLBACK.
           Sept 13 2026. A topic like t-tissues holds seven competencies.
           Crediting every one of them for a single cell-junction card
           made the weak spot board show evidence on competencies the
           student had never been asked about, and let a repair run be
           crossed off by unrelated cards. card-competency-fine.js has
           carried per-card assignments (topicId:cardId) since Aug 22;
           the recall engine deals by them, so scoring must count by
           them too. A card with no fine entry keeps the topic credit. */
        var entry = MAP[tid + ':' + cid] || topicEntry;
        if (!entry.comps || !entry.comps.length) return;
        var r = cards[cid] || {};
        var hist = r.history;
        if (hist && hist.length) {
