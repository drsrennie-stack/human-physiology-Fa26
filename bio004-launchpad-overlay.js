...
(function () {
  ...
  + '.lpov .lp-path{max-width:640px;margin:0 auto 18px;background:#fff;border-radius:18px;padding:16px 16px 14px;box-shadow:0 1px 3px rgba(0,0,0,.08)}'
  + '.lpov .lp-path-h{display:flex;justify-content:space-between;align-items:baseline;gap:10px;flex-wrap:wrap;margin:0 0 10px}'
  + '.lpov .lp-path-h b{font-size:15px;font-weight:800;color:#08101F}.lpov .lp-path-h span{font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#7A2A22}'
  + '.lpov .lp-path-row{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}'
  + '@media (max-width:430px){.lpov .lp-path-row{grid-template-columns:1fr 1fr}}'
  + '.lpov .lp-stage{display:flex;align-items:center;gap:8px;text-decoration:none;color:#08101F;border:1px solid rgba(11,21,48,.14);border-radius:12px;padding:9px 10px;font-weight:800;font-size:13px;transition:transform 200ms ease,box-shadow 200ms ease}'
  + '.lpov .lp-stage:hover{transform:translateY(-2px);box-shadow:0 8px 16px rgba(0,0,0,.10)}'
  + '.lpov .lp-stage i{font-style:normal;display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:50%;font-size:12px;flex:none}'
  + '.lpov .lp-s1 i{background:#08101F;color:#fff}.lpov .lp-s2 i{background:#7A2A22;color:#fff}.lpov .lp-s3 i{background:#DCB45C;color:#08101F}.lpov .lp-s4 i{background:#fff;color:#08101F;box-shadow:inset 0 0 0 2px #08101F}'
  + '@media (prefers-reduced-motion:reduce){.lpov .lp-stage{transition:none}.lpov .lp-stage:hover{transform:none}}'
  ...
  /* Which week it is, off the term start, same arithmetic as the dock. */
  var wkNow = (function () {
    var start = new Date(2026, 7, 17);
    var w = Math.floor((new Date() - start) / (7 * 24 * 3600 * 1000)) + 1;
  ...
  ov.innerHTML = '<div class="lp-wrap">'
    ...
    + '<div class="lp-panel on" data-lp="home">'
    + '<div class="lp-path" aria-label="Your path this week">'
    + '<p class="lp-path-h"><b>Where do I start? This week\'s path.</b><span>Week ' + wkNow + '</span></p>'
    + '<div class="lp-path-row">'
    + '<a class="lp-stage lp-s1" ' + go('week-' + wkNow + '.html#path-learn') + '><i aria-hidden="true">1</i>Learn</a>'
    + '<a class="lp-stage lp-s2" ' + go('week-' + wkNow + '.html#path-practice') + '><i aria-hidden="true">2</i>Practice</a>'
    + '<a class="lp-stage lp-s3" ' + go('week-' + wkNow + '.html#path-apply') + '><i aria-hidden="true">3</i>Apply</a>'
    + '<a class="lp-stage lp-s4" ' + go('week-' + wkNow + '.html#path-check') + '><i aria-hidden="true">4</i>Check</a>'
    + '</div></div>'
    + '<div class="lp-deck">'
    ...
    + door('ic-gold', I.bulb, 'Study', 'Loops, recall cards, brain dump, Mastery OS, exam builder', 'data-lpopen="study"')
    ...
    + door('ic-blue', I.cards, 'Recall cards', 'Straight into what is due', go('recall-rx.html'))
