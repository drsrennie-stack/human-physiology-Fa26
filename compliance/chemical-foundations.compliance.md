# Week 1, Chemical Foundations

**Accessibility compliance notes.** BIO 005 Human Physiology.

## 1. Project

**Project:** BIO 005 slide decks, week 1, Chemical Foundations
**Course:** BIO 005 Human Physiology
**File covered:** `out/slides-p-chemical-foundations.html`, one self contained HTML file
**Built by:** `build-decks.js`, from `engine/ref.css` and `engine/ref.js`, which are the engine of the existing Introduction to Physiology deck, plus `content/slides-p-chemical-foundations.js` for this deck's own words and figures
...
**What this deck covers:** Water, pH, bonding and enzyme kinetics, taught as rates rather than as definitions. 36 slides, 35 reveal cards, 4 inline SVG figures, 31 competency lines, 8 lab annotations, 4 slides carrying a suggested time, 2 comparison tables.
...
| 2.1.1 Keyboard | A | Pass | See section 4. Every one of the 35 reveal cards is a `role="button"` with `tabindex="0"`, reachable in reading order and operable with Enter or Space. Verified by automated test on this file. |
...
| 2.4.6 Headings and labels | AA | Pass | One `h1`, 36 `h2`, one per slide, and 37 `h3` inside cards and rows. No skipped levels. |
...
1. A water molecule and a hydrogen bond to a second water molecule
2. The pH scale from 0 to 14 with a magnified view of the arterial blood range
3. Energy against reaction progress, showing activation energy with and without an enzyme
4. Michaelis Menten curves with no inhibitor, a competitive inhibitor and a non competitive inhibitor

**The two comparison tables.**

- How the two kinds of inhibition differ on every question that matters
- Absorbance change per minute at 340 nm, one enzyme, five temperatures, everything else held constant
...
**8 lab annotations.** A short block headed "You will do this in lab", tying the slide to the bench work. On screen it sits on the `#EDF1F3` tint with a gold left edge. In print the tint is dropped and it prints with a plain border, so it stays legible in monochrome.
...
- One `h1`, 36 `h2`, one per slide, and 37 `h3`. No skipped levels.
