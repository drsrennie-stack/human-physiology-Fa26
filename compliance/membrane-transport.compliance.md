...
**Project:** BIO 005 slide decks, week 2, Membrane Transport
**Course:** BIO 005 Human Physiology
**File covered:** `out/slides-p-membrane-transport.html`, one self contained HTML file
**Built by:** `build-decks.js`, from `engine/ref.css` and `engine/ref.js`, which are the engine of the existing Introduction to Physiology deck, plus `content/slides-p-membrane-transport.js` for this deck's own words and figures
...
**What this deck covers:** Carriers, pumps, saturation, transport maximum and secondary active transport. 34 slides, 42 reveal cards, 4 inline SVG figures, 24 competency lines, 6 lab annotations, 4 slides carrying a suggested time, 3 comparison tables.
...
| 2.1.1 Keyboard | A | Pass | See section 4. Every one of the 42 reveal cards is a `role="button"` with `tabindex="0"`, reachable in reading order and operable with Enter or Space. Verified by automated test on this file. |
...
| 2.4.6 Headings and labels | AA | Pass | One `h1`, 34 `h2`, one per slide, and 43 `h3` inside cards and rows. No skipped levels. |
...
1. Rate of transport plotted against concentration outside the cell
2. The sodium potassium ATPase in the cell membrane
3. Renal glucose titration, filtered, reabsorbed and excreted against plasma glucose
4. A polarised gut cell moving glucose from the lumen to the blood

**The three comparison tables.**

- Simple, channel mediated and carrier mediated diffusion compared
- The four transport types compared on the feature that separates them, the trigger
- Which experiment settles what is in the membrane
...
**24 competency lines.** Each slide that maps to the course competency list ends with a line naming the competencies it covers, each one a link into `competency-study-guide.html`. The links are relative to the deck's own folder, in the same way as the dock script named in section 8. In print they render as plain text with no underline.
...
- One `h1`, 34 `h2`, one per slide, and 43 `h3`. No skipped levels.
